"""
Source Code for Homework 4.a of ECBM E6040, Spring 2016, Columbia University
...
class RNNSLU2(object):
    ...
    """
    def __init__(self, nh, ng, nc, ne, de, cs, normal=True):
        ...
        # bundle
        self.params = [self.emb, self.wx, self.wh,self.wg, self.w1,
                       self.w2,self.bh,self.bg, self.b, self.h0,self.h1]
        idxs = T.imatrix()
        x = self.emb[idxs].reshape((idxs.shape[0], de*cs))
        y_sentence = T.ivector('y_sentence')  # labels
        def recurrence(x_t,h_tm1,h_tm2):
            h_t1 = T.nnet.sigmoid(T.dot(x_t, self.wx)
                                 + T.dot(h_tm1, self.wh) + self.bh)
            h_t2 = T.nnet.sigmoid(T.dot(h_t1, self.w1)
                                 + T.dot(h_tm2, self.wg) + self.bg)
            
            s_t = T.nnet.softmax(T.dot(h_t2, self.w2) + self.b)
            return [h_t1,h_t2,s_t]
        

        [h,g,s], _ = theano.scan(fn=recurrence,
                                sequences=x,
...
def test_rnnslu(**kwargs):
    ...
    emb = theano.function(inputs=[], outputs=rnn.emb)
