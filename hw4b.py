...
import timeit
import os
from hw4_utils import shared_dataset, shuffle, conlleval
...
class RNN(object):
    ...
    def train(self, x, y, learning_rate):
        ...
        self.train(x, labels, learning_rate)
        

    
#TODO: build and train a MLP to learn parity function
def test_mlp_parity(learning_rate=0.01, L1_reg=0.00, L2_reg=0.000, n_epochs=50,
             batch_size=10, n_hidden=500, n_hiddenLayers=1,
             verbose=False,nbit=8,ndata=1000):
    # generate datasets
    train_set = gen_parity_pair(nbit, ndata)
    valid_set = gen_parity_pair(nbit, ndata//2)
    test_set  = gen_parity_pair(nbit, ndata//10)
...
def test_rnn_parity(**kwargs):
    ...
    for e in range(param['nepochs']):
        # shuffle
        shuffle([train_set_x, train_set_y], param['seed'])
